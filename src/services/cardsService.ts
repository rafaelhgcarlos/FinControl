...
export async function deleteUnusedCard(userId: string, cardId: string) {
  const usageChecks = await Promise.all([
    hasCardUsage(userId, collections.cardPurchases, cardId),
    hasCardUsage(userId, collections.cardInstallments, cardId),
    hasCardUsage(userId, collections.cardInvoices, cardId),
    hasCardUsage(userId, collections.cardPayments, cardId),
  ]);
  if (usageChecks.some(Boolean)) {
    throw new Error("Este cartao ja possui compras, faturas, parcelas ou pagamentos. Arquive para preservar o historico.");
  }
