...
export function BudgetsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    await runAction("budget:save", async () => {
      try {
  ...
  async function archive(item: BudgetWithUsage) {
    await runAction(`budget:archive:${item.id}`, async () => {
  ...
  return <>
    ...
    {error ? <ErrorState message={error} /> : loading ? <ListSkeleton label="Carregando orçamentos" /> : budgets.length === 0 ? <Card><EmptyState action={<Button disabled={isActionPending()} onClick={openCreate}><Plus className="h-4 w-4" />Criar orçamento</Button>} title="Nenhum orçamento" description="Crie um limite para acompanhar seus gastos por categoria." icon={<PiggyBank className="h-6 w-6" />} /></Card> :
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{budgets.map((item) => {
        ...
        return <Card key={item.id} className={item.status === "ARCHIVED" ? "opacity-65" : ""}>
          ...
          {item.status !== "ARCHIVED" ? <div className="mt-4 flex gap-2"><Button disabled={isActionPending()} variant="ghost" onClick={() => openEdit(item)}><Edit2 className="h-4 w-4" />Editar</Button><Button disabled={isActionPending()} variant="ghost" onClick={() => void archive(item)}><Archive className="h-4 w-4" />Arquivar</Button></div> : null}
        </Card>;
      })}</div>}
    <Modal isOpen={open} title={editing ? "Editar orcamento" : "Novo orcamento"} onClose={() => setOpen(false)}><form className="grid gap-4" onSubmit={(event) => void submit(event)}>
      ...
      <div className="flex justify-end gap-2"><Button disabled={isActionPending("budget:save")} variant="secondary" onClick={() => setOpen(false)}>Cancelar</Button><Button disabled={isActionPending("budget:save")} type="submit">{isActionPending("budget:save") ? "Salvando..." : "Salvar"}</Button></div>
    </form></Modal>
  </>;
}
