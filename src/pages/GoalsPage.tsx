...
export function GoalsPage() {
  ...
  return <>
    <PageHeader title="Metas financeiras" description="Acompanhe objetivos sem movimentar automaticamente o saldo das contas." action={<Button onClick={openCreate}><Plus className="h-4 w-4" />Nova meta</Button>} />
    {message ? <div className="mb-4"><Toast>{message}</Toast></div> : null}
    {error ? <ErrorState message={error} /> : loading ? <Card><LoadingState label="Carregando metas" /></Card> : goals.length === 0 ? <Card><EmptyState title="Nenhuma meta" description="Defina um objetivo e registre sua evolucao." icon={<Target className="h-6 w-6" />} /></Card> :
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{goals.map((item) => <Card key={item.id} className={item.status === "ARCHIVED" ? "opacity-65" : ""}>
        ...
        {item.status !== "ARCHIVED" ? <div className="mt-4 flex flex-wrap gap-2"><Button variant="ghost" onClick={() => { setAmountGoal(item); setAmountInCents(0); }}><Plus className="h-4 w-4" /><Minus className="h-4 w-4" />Valor</Button><Button variant="ghost" onClick={() => openEdit(item)}><Edit2 className="h-4 w-4" />Editar</Button><Button variant="ghost" onClick={() => void archive(item)}><Archive className="h-4 w-4" />Arquivar</Button></div> : null}
      </Card>)}</div>}
    ...
    <Modal isOpen={Boolean(amountGoal)} title="Atualizar valor da meta" description="Este registro acompanha o progresso e nao movimenta contas." onClose={() => setAmountGoal(null)}><div className="grid gap-4"><FormField id="goal-amount" label="Valor"><CurrencyInput id="goal-amount" value={formatCurrencyFromCents(amountInCents)} onChange={(event) => setAmountInCents(parseCurrencyToCents(event.target.value))} /></FormField><div className="flex justify-end gap-2"><Button variant="secondary" disabled={amountInCents <= 0} onClick={() => void changeAmount(-1)}><Minus className="h-4 w-4" />Retirar</Button><Button disabled={amountInCents <= 0} onClick={() => void changeAmount(1)}><Plus className="h-4 w-4" />Adicionar</Button></div></div></Modal>
  </>;
}
